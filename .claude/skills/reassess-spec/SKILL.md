...
**Reference-count checkpoint**: before Step 3, emit a one-line note with the exact reference count and the tracking decision — e.g. `Reference count: 12 — mental tracking sufficient` or `Reference count: 23 — TaskCreate recommended`. Use an exact integer (not `~20` or `20+`); the >15 threshold-decision must be reproducible. When references form a fuzzy cluster (overlapping symbol names, one doc cited across several sections) below the 50-reference cluster threshold, the integer is a good-faith count and the reproducible artifact is the *threshold decision + tracking choice*, not a precise N — an approximate (`~38 — mental tracking sufficient`) is acceptable only when it cannot flip the >15 decision; near the boundary, resolve to an exact count. Above 50 references, an exact cluster count plus per-category subtotals (e.g. `12 finding-clusters: 32 files + 10 docs + 103 symbols + 15 tests + 6 artifacts`) replaces the single grand-total integer — the threshold decision, not the total, is what must be reproducible. For specs with >15 references spanning unrelated areas, consider `TaskCreate` per-reference tracking; for tightly-clustered sets, mental tracking is acceptable. A fixed closed set checked by one presence grep counts as 1 reference, not N.
