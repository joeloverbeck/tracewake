...
- **Placement correction**: when the Step 1 placement-consistency check produced an approved finding that the spec is misfiled (a PROPOSED spec in `archive/specs/`, or an accepted/Done one still in `specs/`), move the file to its correct location as part of Step 7, then apply the content edits at the new path (and re-Read it there first, since the Edit validator keys on the path). Prefer `git mv`; if the file is **untracked**, `git mv` fails — fall back to a plain `mv`. This is the inverse of the (d)-retroactive archival move below; both are placement corrections, not content edits, and belong in the Step 8 change inventory.
