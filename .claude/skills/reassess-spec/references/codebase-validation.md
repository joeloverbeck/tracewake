...
**Negative/absence verdicts against a cited commit**: when a hardening/audit spec cites an exact target commit and asserts a symbol/path is *absent*, or that a value is `None`/null there, `git grep <symbol> <that-commit>` — not just HEAD — before accepting the negative verdict. Externally deep-researched specs fabricate absence claims as readily as line numbers (the researcher fetches by URL and cannot reliably confirm a non-match), so a "no violation observed / symbol absent" verdict can be false **at the spec's own baseline**. Proving presence-at-the-cited-commit vs. presence-only-at-HEAD distinguishes the two corrections: a *fabricated* absence flips the verdict outright, whereas genuine *baseline drift* (true once, since changed) is noted as drift. The selector/symbol the spec then prescribes a fix for is the first thing to grep at the cited commit.
