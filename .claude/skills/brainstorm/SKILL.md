...
1. **Reference material.**
   ...
   - **Oversized files**: if a reference exceeds the Read token limit, read in sequential offset/limit chunks until fully covered, **and note the chunked read in the summary** (a required item, not an aside). Run a quick `wc -l <path>` first when you suspect a file is large — but note the *token* cap, not line count, is the real trigger: a dense file can exceed the cap well under any obvious line count (e.g. ~500 lines), so the Read's truncation reminder, not `wc -l`, is what tells you a second offset Read is needed. When that happens, finish the chunks before emitting the summary.
...
**Same-turn-write ordering.** When the file write and the recommendation/triage presentation share one turn (the pre-authorization / auto-mode collapse), write the file FIRST and make the full recommendation/triage presentation the turn's **final text message** — the final message is the only text guaranteed shown to the user, and text emitted between tool calls may be dropped. A brief pre-write status note still precedes the write so the action is announced, and the Step 6 menu rides as inline numbered options inside that same final message (see `references/triage-and-deliverables.md` §Non-plan-mode fast-track, Intra-turn ordering, for the full rule). Do not present the substantive triage *before* the write and leave only a summary after it.
...
| Doctrine-tier amendment captured as a spec (proposal, not edited in place) — foundation / architecture / execution / reference | `specs/NNNN_TITLE.md` — proposes substance + home, no ratified wording / no doctrine identifier (`INV-###` for foundation); the spec stages the amendment without enacting it. Approval bar by tier: constitutional sign-off for foundation, ordinary owner approval for architecture/execution/reference; either attaches to the eventual tier edit, not the spec write (see §Deliverable classification) |
...
| Routing / backlog hand-off memo for a later session | `reports/<topic>-routing.md` (or `docs/triage/`) — consolidates settled verdicts/routing; not a research brief, not a triage of new findings (see §Deliverable classification) |
