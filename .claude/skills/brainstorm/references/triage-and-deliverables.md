...
- **Capture the gate baseline when the deliverable will claim it** — when the persisted deliverable will assert the baseline's build/test status (common in hardening-spec lineages whose headers record "all gates verified green at `<commit>`"), the orchestrator runs the project's gate commands itself — in the background, concurrent with the fan-out — so the claim is measured, not assumed; record the result in the deliverable's baseline header.
