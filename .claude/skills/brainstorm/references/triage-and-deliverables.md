...
- **Route all findings to the out-of-report sub-section**, keyed `O<N>` (or the lineage series' IDs per §Per-item identifiers §Lineage series continuation). The answers to the user's questions ARE the findings — emit them as `O1`, `O2`, …, not as synthetic `R<N>` items restating the questions.
