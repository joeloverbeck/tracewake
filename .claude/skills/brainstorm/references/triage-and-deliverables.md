...
- **Amend `docs/0-foundation/02_CONSTITUTIONAL_INVARIANTS.md`** — a constitutional change. The invariants are hard rules; amending one is deliberate and on purpose. Present the amendment explicitly as a constitution change, name the invariant(s) affected and the downstream specs/features the change unblocks or invalidates, and require explicit user sign-off before writing. Skip the design doc — the amendment IS the deliverable. When the amendment is to be *captured as a spec* rather than edited in place (the user asks for a spec, or the reassess/ratification process owns final wording), use the **Foundation-amendment proposal spec** case below instead.

- **Foundation-amendment proposal spec** — when the actionable work is a constitutional (or other foundation-tier) amendment but the deliverable is a *spec* (the user requested a spec, or the reassess/ratification process owns final wording), the spec **proposes** the amendment's substance and home — it does **not** enact doctrine, assign an `INV-###`, or author ratified wording. State explicitly in the spec that it does not exercise spec authority to amend invariants (a spec may not — `docs/4-specs/SPEC_LEDGER.md`); it stages the change for the foundation-amendment process to ratify under that process's authority. The explicit-sign-off gate (see the Amend-`02` case above) attaches to the *eventual foundation edit*, not to writing the proposal spec — the spec write itself is pre-implementation, so under §User pre-authorization presenting the triage/design satisfies the HARD-GATE for the write. Otherwise follow the **System spec** rules below (section set, `Status: PROPOSED`, spec-ID assignment, ledger timing, mandatory Step 6 menu), and flag prominently that the spec's implementation is a constitutional change.
