...
**Hardening passes deliver either shape — the ask decides.** A hardening / anti-contamination pass
produces *either* a recommendation report (ask = "what to change + where", the archetype just below)
*or* a numbered implementation spec (ask = an implementation spec — e.g. a post-feature
foundational-alignment hardening spec in the parallel `specs/NNNN` series like siblings `0046`/`0047`,
staged in `specs/` then archived to `archive/specs/`). When the ask is a **spec**, classify the target as
`hardening / anti-contamination (secondary: new-spec)` to union in the spec reads (`SPEC_LEDGER.md`,
sibling specs, the §7 numbering rules), use determination-plus-conditional **mode (i)** (always produce,
verdict as a section), and apply the numbered-spec numbering/ledger rules above — **not** this report
archetype. Only when the ask is a memo of recommended changes does the report archetype below govern;
do not let the §B hardening row's report-leaning reads imply a hardening target cannot produce a `specs/`
artifact.
...
*recently-completed* work whose pinned keystone commit lags merged HEAD, apply the SKILL.md Step 6
*Auditing recently-completed work* baseline rule — it is deliverable-agnostic and governs spec
deliverables identically) — say so explicitly,
...
**Design / discovery research report (spec-seeding).** Distinct from the cascade/hardening
recommendation report above: when the target is a **new-spec-*driver*** (design what to build next —
a new capability, a subsystem overhaul, a greenfield UX/presentation direction) whose deliverable is a
*recommendation report the user will decompose into as many specs as logically necessary* (SKILL.md
Step 6 *new-spec is a driver type, not a deliverable shape*), the deliverable is a report, **not** a
numbered spec — so the numbered-spec numbering/ledger/epoch rules are **N/A**, and the new-spec
read-set (`SPEC_LEDGER.md`, sibling specs, the touched architecture/execution seams) still applies.
Specify one **new** consolidated `reports/<topic>-research-report.md` (not a replacement). This is a
*greenfield design* report, not a finding→target-doc realignment, so the cascade shape above
(disposition table → forward-routing appendix) does **not** fit; direct Session 2 to a
design-report shape instead: executive summary → current-state assessment (grounded in the named code
seams) → target design → **cross-boundary enrichment routing** (each recommendation that crosses a
crate/tier boundary named and routed to the owning doc) → decision/recommendation (framework, posture,
etc.) → an **ordered spec-decomposition roadmap** (one entry per future spec: one-line objective, scope
boundary, dependencies/ordering, and whether it is single-crate or crate-crossing — **no assigned spec
numbers**, the ledger owns numbering; describe ordering, not `NNNN`) → **routed foundational/doctrine
amendments** if genuinely warranted (substance + home only — what the target doc must own and which
file/section it lands in; **no** paste-ready invariant text, **no** invented `INV-###`/gate/glossary
identifiers; and if none is warranted, say so with reasons rather than manufacturing one) → open
questions & risks (cross-reference the design risk register) → references. Carry a labeled
`assumption:` line if the report's filename or placement is not pinned in the interview.
...
| **hardening / anti-contamination** | `docs/0-foundation/14_ACTOR_KNOWN_COGNITION_TRANSACTION_AND_TRUTH_FIREWALL.md` and the firewall/provenance architecture (`docs/1-architecture/03_*`); the subsystem's foundation + architecture docs; prior hardening specs in `archive/specs/**`; the validation/observability docs. **Deliverable shape:** these reads are report-leaning, but a hardening pass can deliver *either* a recommendation report *or* a numbered implementation spec — the ask decides (see §7 *Hardening passes deliver either shape*). For a spec deliverable, union in the **new-spec** row's reads (`SPEC_LEDGER.md`, sibling specs, numbering) via a `(secondary: new-spec)` classification. |
