...
1. **Write the brief** to `reports/<topic>-research-brief.md`, following the canonical anatomy in `references/brief-template.md` §A (read in Step 1). `<topic>` is a short kebab-case slug of the target. Mind the one-word gap from the deliverable: the brief you write now is `<topic>-research-brief.md`, while the §7 deliverable Session 2 writes is often `<topic>-…-research-report.md` (same slug, `brief` vs. `report`). Do not conflate them when choosing the file path you Write, when naming the §1 manifest pointer, or when specifying §7 — writing the brief to the report's path is a silent foot-gun. The "same slug, `brief` vs. `report`" mapping is 1:1 only for a **single-deliverable** brief. When one brief commissions a **per-tier split** (several reports with distinct slugs — e.g. `reference-tier-alignment-research-report.md` *and* `specs-tier-alignment-research-report.md`), the brief cannot share any one report's slug: name it with a combined campaign slug (e.g. `reference-and-specs-tier-alignment-research-brief.md`), reusing the campaign's archived-precedent brief slug when one exists.
