...
- if `git status` is dirty at write time — pending moves/renames affecting §2 paths, an untracked load-bearing input reproduced inline, **or any other pre-existing uncommitted change (even tracked modifications unrelated to the brief)** — proactively warn the user that committing (or merging/pushing) will move HEAD and invalidate the §1 baseline, triggering the **re-baseline** sequence in Step 6. The hazard is "committing *anything* moves HEAD," not specifically committing a §2-affecting move; pre-existing unrelated edits the user is likely to commit carry the same risk. Surfacing it at authoring time means the user isn't surprised when the bundle needs re-issuing.
