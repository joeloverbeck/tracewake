...
arguments:
  - name: spec_path
    ...
    required: true
  - name: namespace
    description: "Ticket namespace prefix, used as <PREFIX>-<NNN>.md (e.g., P1KERNEL). If omitted, the skill derives one from the spec's Phase number and abbreviated title slug and asks the user to confirm."
    required: false
---
...
5. `<namespace>` is provided, OR derive one from the spec's Phase number and abbreviated title slug (e.g. `specs/0002_PHASE_1_KERNEL_TUI_EVENT_LOG_AND_REPLAY_IMPLEMENTATION_SPEC.md` → `P1KERNEL`) and ask the user to confirm or override before Step 1.
