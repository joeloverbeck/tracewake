...
6. **Severity-tag each finding** — CRITICAL / HIGH / MEDIUM / LOW:
   - **CRITICAL** — skill produces wrong output, corrupts state, or violates an invariant (`INV-NNN`). Fix before next use.
...
- **Invariants**: <aligned / N violations found / N/A — meta-tooling skill>
- **CLAUDE.md**: <aligned / N deviations found / skipped — not present>
[If violations: bullets naming the specific `INV-NNN` or CLAUDE.md section and what conflicts]
...
- **Invariants alignment is mandatory** — any suggestion that would violate an invariant (`INV-NNN`) must be flagged and rejected, even if it would otherwise improve the skill.
