...
use crate::epistemics::SourceRef;
...
use crate::runtime::ValidatedLoadedWorldBootstrap;
use crate::scheduler::{OrderingKey, ProposalSequence, SchedulePhase, SchedulerSourceId};
use crate::time::SimTick;

use crate::content::manifest::ContentManifest;
use crate::content::schema::{FixtureSchema, FixtureScope, WorkplaceSchema};
use crate::content::serialization::SerializationError;
use crate::content::validate::{
    validate_fixture_bytes, ContentValidationFailure, FixtureValidationToken,
};
...
pub struct LoadedFixture {
    ...
    pub canonical_world: crate::state::PhysicalState,
    pub canonical_agent_state: crate::state::AgentState,
    pub epistemic_projection: EpistemicProjection,
    pub seed_event_log: EventLog,
    validation_proof: FixtureValidationToken,
}
...
impl LoadedFixture {
    pub fn into_runtime_bootstrap(
        ...
        let _validation_proof = self.validation_proof;
...
pub fn load_fixture_package(
    ...
    Ok(LoadedFixture {
        ...
        seed_event_log,
        validation_proof: validation_token,
    })
}

fn registry_for_primary_bytes(
    bytes: &[u8],
) -> Result<crate::actions::ActionRegistry, SerializationError> {
...
fn serialize_location(location: &crate::location::Location) -> String {
    match location {
        crate::location::Location::AtPlace(place_id) => {
            format!("place:{}", place_id.as_str())
        }
        crate::location::Location::InContainer(container_id) => {
            format!("container:{}", container_id.as_str())
        }
        crate::location::Location::CarriedBy(actor_id) => {
...
pub fn registry_for_fixture_scope(scope: FixtureScope) -> crate::actions::ActionRegistry {
...
mod tests {
    ...
    use crate::content::serialization::serialize_fixture;
    use crate::ids::{
        ActorId, ContainerId, ControllerId, FixtureId, ItemId, PlaceId, SchemaVersion,
    };
    use crate::location::Location;
    use crate::runtime::{LoadedWorldRuntime, RuntimeReceiptKind};
    use crate::scheduler::WorldAdvanceOrigin;
    use crate::state::VisibilityDefault;
