...
(`reports/tui-experience-overhaul-research-report.md` §3.7, §5.4 Stage D, §7). It adds the real
terminal event loop with `crossterm` (raw mode, alternate screen) that reads key events, translates
them into the Spec `0062` `UiIntent` stream, and redraws the Spec `0064` panes from the current view
model. The shell is strictly an input/output **adapter**; it holds only presentation state.
...
2. **Event loop:** poll `crossterm` key/resize events, translate keys into `UiIntent` values via the
   Spec `0062` reducer, update presentation state or submit through authorized core methods, and redraw
   the Spec `0064` `ratatui` buffer from the current `EmbodiedViewModel`.
...
- **Depends on:** `0061` (screen model), `0062` (intent reducer), `0064` (panes/buffer).
- **Blocks:** `0068` (debug dashboard runs inside the shell), and contributes to Spec `0069`.
- **Parallelizable with:** `0064` after the screen model stabilizes.
