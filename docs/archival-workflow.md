...
Use this as the canonical, single-source archival policy for tickets, specs, design/plan docs, and reports.
...
7. Confirm the original path no longer exists in its source folder (`tickets/`, `specs/` or `docs/4-specs/`, `docs/plans/`, or `reports/`).
8. If the moved report is **load-bearing** — referenced by compiled tests (`include_str!`) or cited as canonical conformance evidence in `docs/**` — retarget every such reference to the new `archive/reports/` path in the same change, then keep the archived file frozen: do not re-move or rename it, since the build and doctrine now depend on that exact path. (Acceptance artifacts and mutation-baseline ledgers for archived specs are the usual case.)
