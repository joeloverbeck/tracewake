...
|---|---|---|
| 0050/0051 loaded-world temporal conformance | Human wait/continuation, no-human advancement, temporal frontier restoration, and interval summaries must consume the same core world-step transaction, replay-derived frontier, positive actor-known interval delta, and core-owned final interval product; neither TUI code nor prose/debug summaries may author time, ordinary work, replay success, or holder knowledge. | `archive/tickets/0050FOUCONSEC-001.md` through `archive/tickets/0050FOUCONSEC-011.md` replace the overbroad 0048 reachability locks with production-path witnesses, and `archive/tickets/0051FOUCONTHI-001.md` through `archive/tickets/0051FOUCONTHI-010.md` revalidate that evidence against the repaired runtime. The current production entry point is `LoadedWorldRuntime::from_loaded_world`, which rebuilds scheduler authority from loaded fixture/content state, derives loaded actors and declared processes, and crosses the TUI boundary through `TuiApp::submit_entry_with_world_advance` / command-loop world-advance paths rather than client-authored mutation. Current evidence is split by scope: `crates/tracewake-core/tests/generative_lock.rs` proves loaded runtime construction, mixed schedules, order invariance, prefix replay, hidden-world noninterference, and duplicate marker failure; `crates/tracewake-content` loader tests prove loaded content hands off derived runtime due work; `crates/tracewake-core/tests/world_step_coordinator.rs` proves loaded actor/process discovery, actor-transaction choreography, restored eligibility, process events, and actor census effects; `crates/tracewake-core/tests/replay_temporal_frontier.rs` and replay report tests prove temporal violations fail aggregate replay; `crates/tracewake-core/tests/holder_known_interval_projection.rs` and `crates/tracewake-core/tests/salient_stop_actor_known.rs` prove holder-known interval products, subject-separated observation replacement, and stop salience; `crates/tracewake-core/tests/negative_fixture_runner.rs` proves compile-time authority boundaries; `crates/tracewake-tui/tests/playable_capability_parity.rs`, `crates/tracewake-tui/tests/parity_adversarial.rs`, and `crates/tracewake-tui/tests/command_loop_session.rs` prove typed-before-rendered TUI parity, adversarial evidence-class failure, and public command crossing. `archive/tickets/0051FOUCONTHI-009.md` records the runtime-constructor CI lane; `archive/tickets/0051FOUCONTHI-010.md` records focused mutation sensitivity plus a standing disposition of `3275` selected, `2549` caught, `703` unviable, `23` missed, `0` timeout. `reports/0048_foundational_conformance_hardening_acceptance.md` remains historical context, not the current production-reachability proof. |
