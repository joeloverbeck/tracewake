...
| cognition-substrate source | No-human cognition is built from event-log/source-event evidence, not raw authoritative tables; the broader cognition-substrate split remains explicitly deferred. | `NoHumanActorKnownSurfaceBuilder::from_event_log` is the no-human surface entry point; the remaining substrate follow-up is tracked by `tickets/0016PHA3ANEEACC-015.md`. |
