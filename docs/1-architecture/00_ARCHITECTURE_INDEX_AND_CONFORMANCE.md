...
| 0020/0022 mutation-baseline governance | Accepted mutation misses are pinned by normalized count/hash and by a reviewed per-entry disposition ledger with closed disposition tags; the 0022 triage pass must classify every remaining entry as either current accepted baseline or a concrete `warrants-test:<ticket-id>` follow-up. Unledgered misses, stale ledger entries, unsupported tags, missing `warrants-test` tickets, deferred-test prose in `justified-baseline`, and bulk-repeated rationales fail the guard. | `.cargo/mutants-baseline-misses.txt`, `reports/0020_mutants_baseline_disposition.md`, `MUTANTS_BASELINE_NORMALIZED_COUNT`, `MUTANTS_BASELINE_NORMALIZED_FNV1A64`, and `mutation_baseline_misses_are_pinned_and_ledgered` lock the accepted baseline surface with synthetic unledgered append, unrecorded floor raise, repeated rationale, missing-ticket, and deferred-test-language failures. |
