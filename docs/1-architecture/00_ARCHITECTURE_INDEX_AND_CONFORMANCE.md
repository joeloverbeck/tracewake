...
|---|---|---|
| 0048 loaded-world temporal conformance | Human wait/continuation, no-human advancement, temporal frontier restoration, and interval summaries must consume the same core world-step transaction, replay-derived frontier, and positive actor-known interval delta; neither TUI code nor prose/debug summaries may author time, ordinary work, or holder knowledge. | `archive/tickets/0048FOUCONHAR-001.md` through `archive/tickets/0048FOUCONHAR-007.md`, `reports/0048_foundational_conformance_hardening_acceptance.md`, `crates/tracewake-core/tests/replay_temporal_frontier.rs`, `crates/tracewake-core/tests/world_step_coordinator.rs`, `crates/tracewake-core/tests/holder_known_interval_projection.rs`, `crates/tracewake-core/tests/salient_stop_actor_known.rs`, `crates/tracewake-core/tests/generative_lock.rs`, `crates/tracewake-core/tests/reservation_body_exclusive_census.rs`, and `crates/tracewake-tui/tests/playable_capability_parity.rs` lock replay frontier reconstruction, holder-known interval provenance, loaded-world actor/process differential, duration/accounting parity, and measured typed-before-rendered TUI output. |
