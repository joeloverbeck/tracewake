...
|---|---|---|
| 0050/0051/0052/0056 loaded-world temporal conformance | Human wait/continuation, no-human advancement, temporal frontier restoration, interval summaries, validated-content bootstrap, and debug/operator commands must consume the same sealed core world-step and command boundary; neither TUI code nor prose/debug summaries may author time, ordinary work, replay success, holder knowledge, runtime bootstrap authority, or debug authority. | `archive/tickets/0050FOUCONSEC-001.md` through `archive/tickets/0050FOUCONSEC-011.md`, `archive/tickets/0051FOUCONTHI-001.md` through `archive/tickets/0051FOUCONTHI-010.md`, and `archive/tickets/0052FOUCONFOU-001.md` through `archive/tickets/0052FOUCONFOU-011.md` provide the prior temporal production boundary. `archive/tickets/0056FOUCONSEV-001.md`, `archive/tickets/0056FOUCONSEV-002.md`, `archive/tickets/0056FOUCONSEV-005.md`, and `archive/tickets/0056FOUCONSEV-006.md` truth the current sealed end-state: the production entry point is `LoadedWorldRuntime::from_bootstrap(ValidatedLoadedWorldBootstrap, SimTick)`, produced by validated content handoff, and public crossing is through `TuiApp::submit_semantic_action`, `TuiApp::advance_until`, and command-loop commands that dispatch closed `RuntimeCommand` values rather than client-authored scheduler mutation. Debug/operator commands require a presented `DebugSessionAuthority` obtained through an operator entrypoint, not ordinary embodied self-bind. Observed effects are committed actor/process events, process ancestry, restored replay authority, one disposition per loaded actor, sealed embodied/debug receipts, read-only interval products, replay verdicts, and rendered normal/debug output. Current evidence is split by scope: `crates/tracewake-core/tests/generative_lock.rs` proves production-constructor mixed schedules, order invariance, prefix replay, hidden-world noninterference, and duplicate marker failure; `crates/tracewake-content` loader tests prove content hands off derived runtime due work; `crates/tracewake-core/tests/world_step_coordinator.rs` proves loaded actor/process discovery, actor-transaction choreography, restored eligibility, process events, and actor census effects; `crates/tracewake-core/tests/replay_temporal_frontier.rs` and replay report tests prove temporal violations fail aggregate replay; `crates/tracewake-core/tests/holder_known_interval_projection.rs` and `crates/tracewake-core/tests/salient_stop_actor_known.rs` prove holder-known interval products, subject-separated observation replacement, and stop salience; `crates/tracewake-core/tests/negative_fixture_runner.rs` proves compile-time authority boundaries under default and all-feature combinations; `crates/tracewake-tui/tests/command_loop_session.rs`, `crates/tracewake-tui/tests/playable_capability_parity.rs`, `crates/tracewake-tui/tests/embodied_flow.rs`, and `crates/tracewake-tui/tests/parity_adversarial.rs` prove public command crossing, typed-before-rendered parity, sealed receipt rendering, and adversarial evidence-class failure. Current sensitivity is the final clean standing mutation campaign at `37062d6`: `3451` mutants tested, `2681` caught, `770` unviable, `0` missed, and `0` timeout; no historical 0054 count is treated as current green evidence. `reports/0048_foundational_conformance_hardening_acceptance.md` and earlier standing dispositions remain historical context, not the current production-reachability proof. |
