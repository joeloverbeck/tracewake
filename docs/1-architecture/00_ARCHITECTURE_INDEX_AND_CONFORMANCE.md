...
| 0020 derived apply-arm payload census | Every `events/apply.rs` write into a checksum-covered `AgentState` map must be found by a derived source census and must either require a supported payload/trace/diagnostic schema version or carry an explicit typed-column-closure exemption. | `materialized_agent_apply_arms_require_payload_schema_version` derives covered map writes from `AGENT_STATE_CHECKSUM_COVERAGE` and `events/apply.rs`; `TYPED_COLUMN_CLOSURE_EXEMPTIONS` records the need/intention/routine typed-column dispositions; the synthetic regression case fails on an unversioned, unexempted covered-map write. |
| 0020 mutation-perimeter completion | The lock-layer mutation perimeter includes actor cognition, scheduler, projections, action pipeline, and the guarded eat/sleep/work action definitions across scheduled and in-diff CI jobs. | `.github/workflows/ci.yml` schedules `cargo mutants` over the full filter set; `mutation_perimeter_matches_duration_action_rationale_and_ci_filters` checks the scheduled filters, in-diff guarded-path regex canaries, CI failure semantics, and action-source classification rationale. |
| 0020 dead embodied-field sweep | Every `Option`/collection field on embodied view/status structs must have a reachable non-default producer or a cited external-producer/deferral entry; dead view-model surfaces are unfinished mechanics, not harmless shape. | `embodied_view_option_and_collection_fields_have_reachable_producers` derives the field inventory from `view_models.rs`, scans production producers, requires cited `EMBODIED_SURFACE_FIELD_PRODUCERS` entries for external population such as TUI notebook attachment, and fails on a synthetic hardwired-default field. |
| 0020 mutation-baseline governance | Accepted mutation misses are pinned by normalized count/hash and by a reviewed per-entry disposition ledger; an appended miss without ledger update fails the guard. | `.cargo/mutants-baseline-misses.txt`, `reports/0020_mutants_baseline_disposition.md`, and `mutation_baseline_misses_are_pinned_and_ledgered` lock the accepted baseline surface and synthetic unledgered append failure. |
...
| 0019 mutation-perimeter honesty | The mutation lock layer includes guarded action-definition builders, treats cargo-mutants tool failure as a failed gate, and runs in-diff mutation checks for guarded PR and direct-push changes. | `.cargo/mutants.toml` no longer excludes `actions/defs/**`; CI filters include `actions/defs/eat.rs`, `actions/defs/sleep.rs`, and `actions/defs/work.rs`; `mutation_perimeter_matches_duration_action_rationale_and_ci_filters` checks the mutants config, CI failure semantics, guarded push path, and classification rationale. |
