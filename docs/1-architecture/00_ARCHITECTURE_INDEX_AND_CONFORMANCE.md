...
Time-control and interval-summary capabilities are included in this standing
parity posture. Human world advance is a consumer of the existing tick-charge
attribution and shared open-duration authorities below; it does not create a
parallel accounting or duration conformance family.

## Phase 3A evented cognition conformance

| Surface | Required allowed source | Required evidence |
|---|---|---|
| 0050/0051/0052 loaded-world temporal conformance | Human wait/continuation, no-human advancement, temporal frontier restoration, and interval summaries must consume the same core world-step transaction, replay-derived frontier, positive actor-known interval delta, and core-owned final interval product; neither TUI code nor prose/debug summaries may author time, ordinary work, replay success, or holder knowledge. | `archive/tickets/0050FOUCONSEC-001.md` through `archive/tickets/0050FOUCONSEC-011.md` replace the overbroad 0048 reachability locks, `archive/tickets/0051FOUCONTHI-001.md` through `archive/tickets/0051FOUCONTHI-010.md` preserve prior temporal evidence, and `archive/tickets/0052FOUCONFOU-001.md` through `archive/tickets/0052FOUCONFOU-011.md` close the current production boundary. The current production entry point is `LoadedWorldRuntime::from_bootstrap(LoadedWorldBootstrap, SimTick)`, produced from validated content by `load_fixture_package(...).into_runtime_bootstrap(registry)` in `TuiApp::from_golden`; public crossing is through `TuiApp::submit_semantic_action`, `TuiApp::advance_until`, and command-loop commands that dispatch closed `RuntimeCommand` values rather than client-authored scheduler mutation. Observed effects are committed actor/process events, process ancestry, restored replay authority, one disposition per loaded actor, sealed embodied/debug receipts, read-only interval products, replay verdicts, and rendered normal/debug output. Current evidence is split by scope: `crates/tracewake-core/tests/generative_lock.rs` proves production-constructor mixed schedules, order invariance, prefix replay, hidden-world noninterference, and duplicate marker failure; `crates/tracewake-content` loader tests prove content hands off derived runtime due work; `crates/tracewake-core/tests/world_step_coordinator.rs` proves loaded actor/process discovery, actor-transaction choreography, restored eligibility, process events, and actor census effects; `crates/tracewake-core/tests/replay_temporal_frontier.rs` and replay report tests prove temporal violations fail aggregate replay; `crates/tracewake-core/tests/holder_known_interval_projection.rs` and `crates/tracewake-core/tests/salient_stop_actor_known.rs` prove holder-known interval products, subject-separated observation replacement, and stop salience; `crates/tracewake-core/tests/negative_fixture_runner.rs` proves compile-time authority boundaries under default and all-feature combinations; `crates/tracewake-tui/tests/command_loop_session.rs`, `crates/tracewake-tui/tests/playable_capability_parity.rs`, `crates/tracewake-tui/tests/embodied_flow.rs`, and `crates/tracewake-tui/tests/parity_adversarial.rs` prove public command crossing, typed-before-rendered parity, sealed receipt rendering, and adversarial evidence-class failure. Sensitivity is recorded by `archive/tickets/0052FOUCONFOU-009.md` through `archive/tickets/0052FOUCONFOU-011.md`: the required checks are `public-boundary conformance` and `mutation shard reconciliation (lock layer)`, and `archive/tickets/0052FOUCONFOU-010.md` records `3400` selected, `2645` caught, `748` unviable, `7` routed-forward `food_source_fact_supersedes` misses, and `0` timeouts. The canonical perimeter is not called fully green while those routed-forward food-source mutants survive. `reports/0048_foundational_conformance_hardening_acceptance.md` and the 0051 standing disposition remain historical context, not the current production-reachability proof. |
...
| 0017 tick-charge attribution authority | Every tick-delta need charge, including action-emitted awake deltas and human world-advance ticks, is reconciled by counted `(actor, need, tick)` occurrences rather than regime-label sets. | `assert_single_tick_delta_charge`, `AgentState::need_tick_charges`, `assert_no_duplicate_need_regime_charges`, `assert_single_need_charge_per_actor_tick_kind`, and `wait_then_window_passive_charges_each_tick_once_001` lock the one-charge-per-tick rule. Human world-advance evidence must consume this authority instead of adding a second accounting path. |
| 0017 shared open-duration authority | Open body-exclusive duration state uses event-cause start ids across scheduler, pipeline, need-accounting, and human world-advance consumers; duplicate or orphan terminals are not silently reconciled. | `open_body_exclusive_starts`, `body_exclusive_reservation_conflict`, `terminal_ticks_by_start`, `duplicate_duration_terminal_violations`, and `duplicate_duration_terminal_poisons_rebuild_001` prove shared keying and fail-closed duplicate terminal handling. Human wait/continuation evidence must consume this authority instead of keeping a separate pending-duration source. |
...
| 0020/0022/0039 mutation-perimeter completion | The lock-layer mutation perimeter includes the standing SPINE-CERT source union across scheduled and in-diff CI jobs; scheduled/manual mutation runs must not be canceled mid-flight, and workflow concurrency must isolate event classes so scheduled ratchets cannot be canceled by push runs. | `.cargo/mutants.toml` is the authoritative checked-in allow-list with `test_workspace = true`; `.github/workflows/ci.yml` captures every `cargo mutants` status, lets accepted exit-2 misses flow into the baseline ratchet, invokes the scheduled run through the checked-in config without a divergent `-f` list, detects in-diff changes across the same SPINE perimeter, includes `github.event_name` in the concurrency group, and exempts `schedule`/`workflow_dispatch` from concurrency cancellation; `ci_workflow_guards_cover_workflow_integrity` includes synthetic failures for status-capture removal, `|| true`, missing standing-perimeter entries, `test_workspace` removal, reintroduced scheduled `-f` filters, missing baseline enforcement, missing artifact retention, and event-name concurrency removal. |
...
| 0019/0039 mutation-perimeter honesty | The mutation lock layer includes guarded action-definition builders, treats cargo-mutants tool failure as a failed gate, and separates true mutation-perimeter rationale from non-perimeter action-code rationale. | `.cargo/mutants.toml` no longer excludes SPINE sources; it explicitly examines the standing SPINE-CERT perimeter, including `actions/defs/eat.rs`, `actions/defs/sleep.rs`, and `actions/defs/work.rs`; `CORE_ACTION_MUTATION_PERIMETER_RATIONALE` is used only for perimeter paths, and `ci_workflow_guards_cover_workflow_integrity` checks the mutants config, CI failure semantics, in-diff trigger surface, scheduled checked-config command, and baseline/artifact enforcement. |
