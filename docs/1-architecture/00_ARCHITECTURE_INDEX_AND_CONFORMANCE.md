...
| 0019 mutation-perimeter honesty | The mutation lock layer includes the sleep/work duration-definition builders, treats cargo-mutants tool failure as a failed gate, and runs in-diff mutation checks for guarded PR and direct-push changes. | `.cargo/mutants.toml` no longer excludes `actions/defs/**`; CI filters include `actions/defs/sleep.rs` and `actions/defs/work.rs`; `mutation_perimeter_matches_duration_action_rationale_and_ci_filters` checks the mutants config, CI failure semantics, guarded push path, and classification rationale. |
