...
|---|---|---|
| 0050 loaded-world temporal conformance | Human wait/continuation, no-human advancement, temporal frontier restoration, and interval summaries must consume the same core world-step transaction, replay-derived frontier, positive actor-known interval delta, and core-owned final interval product; neither TUI code nor prose/debug summaries may author time, ordinary work, replay success, or holder knowledge. | `archive/tickets/0050FOUCONSEC-001.md` through `archive/tickets/0050FOUCONSEC-011.md` replace the overbroad 0048 reachability locks with production-path witnesses: `DeterministicScheduler::transact_world_one_tick`, `due_loaded_actor_ids`, `due_process_invocations`, closed `ActorDecisionTransactionOutcome` consumption, `EventLogError::DuplicateEventId`, `ReplayTemporalVerdict`, `actor_known_interval_delta`, and typed interval salience. Current evidence is split by scope: `crates/tracewake-core/tests/world_step_coordinator.rs` proves loaded actor/process discovery and actor-transaction choreography; `crates/tracewake-core/tests/replay_temporal_frontier.rs` and replay report tests prove temporal violations fail aggregate replay; `crates/tracewake-core/tests/holder_known_interval_projection.rs` and `crates/tracewake-core/tests/salient_stop_actor_known.rs` prove holder-known interval products and stop salience; `crates/tracewake-core/tests/generative_lock.rs` proves production-shaped mixed schedules, order invariance, prefix replay, hidden-world noninterference, and duplicate marker failure; `crates/tracewake-core/tests/negative_fixture_runner.rs` proves compile-time authority boundaries; `crates/tracewake-tui/tests/playable_capability_parity.rs` and `crates/tracewake-tui/tests/parity_adversarial.rs` prove typed-before-rendered TUI parity and adversarial evidence-class failure. `reports/0048_foundational_conformance_hardening_acceptance.md` remains historical context, not the current production-reachability proof. |
