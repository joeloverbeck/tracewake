...
| 0018 generative reachability contract | Bounded generated scheduled no-human sequences now exercise wait, eat, sleep, work, duration terminals, interruptions, replay, prefix replay, payload, checksum, and single-charge locks. Supersedes the 0017 concession that duration terminal reachability was covered only by targeted fixtures. | `GeneratedActionKind`, `ActionMask`, expanded `GENERATIVE_SEEDS`, and `generated_sequences_replay_and_satisfy_metamorphic_locks` assert nonzero reachability and corpus diversity for Phase 3A regimes. |
