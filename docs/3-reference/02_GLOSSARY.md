...
| Affect-bearing memory | Canonical, context-bound | A memory whose later salience or priority influence is backed by modeled experience and provenance. It may explain attention; it must not reveal hidden truth, overwrite belief, or select targets without actor-known support. |
| Affect pressure | Canonical, context-bound | A source-backed pressure on attention, willingness, or priority. It stays subordinate to actor-known cognition, validation, and replayable transaction proof; concrete affect depth belongs to future scoped specs. |
...
| Aggregate success | Canonical, context-bound | A diagnostic summary over many actors, regions, institutions, LOD classes, or runs. It can help review budget and fairness behavior, but it must not hide repeated starvation or become an objective for simulation behavior. |
...
| Bounded affect | Canonical, context-bound | Review term for modeled affect that can influence attention or priority only through source-backed state and bounded transaction inputs. It may not become hidden-truth targeting, drama forcing, belief overwrite, or a planning bypass. |
| Budget exhaustion | Canonical, context-bound | Diagnostic condition where a scheduler, planner, or cognition budget cannot complete the attempted work. It must be visible enough to distinguish deferred or degraded cognition from successful reasoning. Concrete budgets and fairness formulas belong to future scoped specs. |
...
| Custody lineage | Canonical, context-bound | The event-sourced ancestry for who held, stored, owed, controlled, disclosed, hid, discovered, transferred, consumed, spoiled, split, or merged an item, lot, stock, payment, debt, or record. Display totals and ledgers summarize lineage; they do not replace it. |
...
| Deferred cognition | Canonical, context-bound | A diagnostic outcome where cognition-like work is postponed rather than completed. It must remain reviewable by holder, cause, and later consequence, and must not silently erase non-player-adjacent reasoning for performance or drama. |
| Degraded cognition | Canonical, context-bound | A diagnostic outcome where cognition-like work runs at reduced fidelity. It must preserve the truth firewall, holder-known inputs, and replayable explanation; degradation must not grant omniscience or mask starvation. |
...
| Domain-pack assumption | Canonical, context-bound | An explicit content-pack premise that shapes institutions, access, credibility, norms, social position, or bias. It must be reviewable as authored content and modeled procedure, not hidden moral truth or kernel ontology. |
...
| Event/replay time | Canonical, context-bound | Authoritative ordering and duration evidence used by event logs, replay, validation, diagnostics, and projections. It may validate and explain; it may not become a holder's premise unless exposed through modeled holder-known or institution-known channels under `INV-112`. Concrete temporal surfaces belong to future scoped specs. |
...
| Fairness envelope | Canonical, context-bound | Diagnostic boundary for whether scheduling, planning, or cognition budgets repeatedly defer, skip, or degrade particular holders, regions, institutions, or LOD classes. It is review vocabulary, not a formula or target. |
| False certification | Restricted | The evidence-honesty relapse where a staged abstraction, omission, or limited witness is treated as proof of behavior that was not implemented or observed. Use this to review acceptance claims; do not use it to define gate semantics. |
...
| Holder-known temporal claim | Canonical, context-bound | A holder-specific claim about ordering, interval, freshness, deadline, expectation, or procedural availability that reached the holder through modeled observation, memory, record, routine, notice, testimony, or procedure. It may guide planning only inside that holder-known boundary. |
...
| Information ancestry | Canonical, context-bound | The source chain explaining how a holder, institution, summary, projection, or promoted LOD entity could know or infer a claim. Temporal and quantity summaries need information ancestry as well as event ancestry when they become holder-facing. |
...
| Institution-known procedural time | Canonical, context-bound | The temporal state available to an institution or role procedure through modeled records, office state, notices, assignments, schedules, observations, and procedure history. It may drive institutional procedure only as institution-known context, not as raw scheduler truth. |
...
| Learned expectation | Canonical, context-bound | A source-backed, defeasible actor or holder state derived from modeled experience. It is distinct from a record or memory, cites what experience supports it, carries scope and freshness, and must not become timeless truth or a cache of validation/debug outcomes. |
...
| LOD summary cadence | Canonical, context-bound | The declared temporal rhythm or granularity of a summary process at lower detail. It may support replay and promotion only when temporal ancestry and information ancestry survive; concrete cadence choices belong to future scoped specs. |
...
| Protected/social category | Restricted | A review concept for content-pack assumptions, access, credibility, and harm analysis. It is not a kernel taxonomy, morality oracle, or scoring system; concrete categories and vehicles belong to future domain-pack specs. |
...
| Quantity granularity | Canonical, context-bound | Review term for whether a thing is modeled as a unique item, a countable lot, a divisible stock, a ledger amount, or a display total. The chosen granularity must preserve event authority and custody lineage; concrete units and schemas belong to future scoped specs. |
...
| Scheduler budget | Canonical, context-bound | Diagnostic limit on scheduler or cognition work. It may constrain processing and explain deferral or degradation, but it must not become a hidden director or a source of holder knowledge. Concrete budget values and algorithms belong to future scoped specs. |
| Scheduler time | Canonical, context-bound | The scheduler's ordering and wakeup authority for validation, action duration, interruptions, no-human stepping, and replay. It orders and validates; it may not plan for a holder unless transformed into holder-known or institution-known temporal claims through modeled channels. |
...
| Skipped cognition | Canonical, context-bound | A diagnostic outcome where a holder's cognition-like work was not run. It must be explicit enough for starvation and fairness review and must not be hidden by aggregate success. |
...
| Staged abstraction | Canonical, context-bound | A declared, bounded omission or simplification in an acceptance artifact or scoped proof package. It is honest only when the omitted behavior and future blocker are named; it must not be treated as proof of the omitted future behavior. |
| Starvation | Canonical, context-bound | Repeated deferral, skipped cognition, or degraded cognition for a holder, region, institution, or LOD class despite aggregate success elsewhere. It is a diagnostic and review concern, not a player-proximity priority rule. |
...
| Freshness/staleness as source-backed status | Canonical, context-bound | Review term for whether a claim, record, memory, procedure state, or projection remains usable for a holder because its source, acquisition history, contradictions, and validity window support that status. It is not automatic ground-truth correction. Concrete thresholds and surface labels belong to future scoped specs. |
...
| Temporal ancestry | Canonical, context-bound | The preserved event/replay and source chain that explains how a temporal claim, schedule, summary, projection, or promoted state got its ordering and availability. It must survive summaries, snapshots, projections, and compaction where later cognition or review depends on it. |
| Temporal authority | Canonical, context-bound | The `INV-112` boundary: event/replay and scheduler time may order, validate, and explain, while cognition, procedure, views, speech, leads, and LOD promotion may use temporal facts only as holder-known or institution-known temporal claims. Reference names the boundary; concrete temporal vocabulary belongs to future scoped specs. |
| Temporal firewall | Canonical, context-bound | The truth firewall applied to time. Raw scheduler, replay, debug, host, or transcript time may not become diegetic cognition, embodied view content, speech meaning, lead urgency, or institutional procedure state except through modeled holder-known or institution-known channels. |
| Time acceleration | Canonical, context-bound | A scheduling or presentation capability that advances simulation time faster than ordinary interaction. It must preserve replay, event authority, holder-known temporal claims, and embodied/debug separation; concrete speeds and UI strings belong to future scoped specs. |
...
| Validity window | Canonical, context-bound | The source-backed interval or applicability boundary within which a holder, institution, projection, record, or procedure may treat information as usable. It is a review concept; concrete durations and thresholds belong to future scoped specs. |
...
| Overclaiming | Restricted | Evidence-honesty wording for claiming more than the cited implementation, witness, artifact, or staged abstraction proves. Prefer false certification when the overclaim specifically treats omitted future behavior as already proven. |
...
| Practical bias | Canonical, context-bound | Review term for modeled credibility, access, procedure, social position, and content-pack assumptions that can shape partial or wrong outcomes. It must be explicit, source-backed, and domain-pack-bound, not hidden moral truth, author verdict, or a genre-neutral kernel category. |
