...
- **0050 evidence/status:** The current loaded-world/time-control reachability proof is the 0050 production-path replacement, not the historical 0048 harness-injected population. `archive/tickets/0050FOUCONSEC-010.md` maps the repaired behavior witnesses to `world_step_coordinator.rs`, `generative_lock.rs`, `negative_fixture_runner.rs`, `playable_capability_parity.rs`, and `parity_adversarial.rs`; `archive/tickets/0050FOUCONSEC-011.md` records focused 0049 preservation campaigns and focused 0050 mutation campaigns. The same ticket records that the configured standing campaign still has 48 missed mutants and one timeout, so standing-perimeter certification remains a separate remediation concern.
...
- **0050 evidence/status:** `archive/tickets/0050FOUCONSEC-001.md` through `archive/tickets/0050FOUCONSEC-010.md` enumerate the full correction family across loaded-actor discovery, declared process discovery, request cutover, compile-fail boundaries, actor transaction outcome consumption, core interval ownership, event-id uniqueness, replay temporal verdicts, typed interval salience, and replacement reachability locks. The 0050 rows should be cited for current defect-family closure; the 0048 acceptance remains historical context.
...
- **0050 evidence/status:** The current non-vacuity posture for this seam is behavior-first: `generative_lock.rs` uses production-shaped mixed schedules, prefix replay, hidden-world pairs, order invariance, and duplicate-marker failure; `parity_adversarial.rs` fails when actor invocation, process invocation, temporal markers, or holder-known evidence are removed; `negative_fixture_runner.rs` keeps due-work, actor-step outcome, TUI perception, and debug-to-embodied authority boundaries compile-fail. Source/topology scans remain secondary alarms and are not counted as behavior proof.
