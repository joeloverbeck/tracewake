...
- **0051 evidence/status:** The current loaded-world/time-control reachability proof is the 0051 production-runtime replacement, not the historical 0048 harness-injected population. `LoadedWorldRuntime::from_loaded_world` is the production constructor; public wait/continue world-advance commands cross the TUI/core boundary; observed effects are committed actor/process events, restored eligibility, actor census changes, replay verdicts, and core-owned interval products. `archive/tickets/0051FOUCONTHI-009.md` records the runtime-constructor CI witness and loaded-content handoff; `archive/tickets/0051FOUCONTHI-010.md` records focused mutation sensitivity and a standing disposition of `3275` selected, `2549` caught, `703` unviable, `23` missed, `0` timeout. Standing-perimeter certification remains a separate remediation concern.
...
- **0051 evidence/status:** `archive/tickets/0051FOUCONTHI-001.md` through `archive/tickets/0051FOUCONTHI-010.md` enumerate the current correction family across runtime construction, loaded-actor discovery, declared process discovery, request cutover, compile-fail boundaries, actor transaction outcome consumption, core interval ownership, event-id uniqueness, replay temporal verdicts, typed interval salience, replacement reachability locks, and mutation evidence disposition. The 0051 rows should be cited for current defect-family closure; the 0048 acceptance and 0050 rows remain historical/prior-pass context.
...
- **0051 evidence/status:** The current non-vacuity posture for this seam is behavior-first: `generative_lock.rs` uses `LoadedWorldRuntime::from_loaded_world`, production-shaped mixed schedules, prefix replay, hidden-world pairs, order invariance, and duplicate-marker failure; `parity_adversarial.rs` fails when actor invocation, process invocation, temporal markers, or holder-known evidence are removed; `negative_fixture_runner.rs` keeps due-work, actor-step outcome, TUI perception, and debug-to-embodied authority boundaries compile-fail; `archive/tickets/0051FOUCONTHI-010.md` records mutation sensitivity for the projection/interval evidence and the remaining standing misses. Source/topology scans remain secondary alarms and are not counted as behavior proof.
