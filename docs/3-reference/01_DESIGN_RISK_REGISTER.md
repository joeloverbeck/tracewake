...
- **0052 evidence/status:** The current loaded-world/time-control reachability proof is the 0052 production-boundary replacement, not the historical 0048 harness-injected population or the narrower 0051 runtime row. `LoadedWorldRuntime::from_bootstrap` is the production constructor from a validated `LoadedWorldBootstrap`; public `TuiApp::submit_semantic_action`, `TuiApp::advance_until`, and command-loop commands cross the TUI/core boundary through closed `RuntimeCommand` dispatch; observed effects are committed actor/process events, restored eligibility, actor census changes, replay verdicts, sealed receipts, rendered output, and core-owned interval products. `archive/tickets/0052FOUCONFOU-009.md` records the production-boundary corpus, `archive/tickets/0052FOUCONFOU-011.md` records the `public-boundary conformance` and `mutation shard reconciliation (lock layer)` required-check names for operational confirmation, and `archive/tickets/0052FOUCONFOU-010.md` records `3400` selected, `2645` caught, `748` unviable, `7` routed-forward `food_source_fact_supersedes` misses, and `0` timeout. The row is zero-miss/zero-timeout for the 0052 in-surface perimeter but not an unqualified green canonical standing-perimeter certification while the food-source family survives.
- **0053 mitigation status:** The 0053 manifest and wording-guard mechanism makes reachability and closure claims fail closed when `open`, `pending-governance`, unbounded `routed-forward`, `historical-only`, sampled, or observer-only evidence is counted as pass. Future 0053 acceptance rows must name the emitting path, status-manifest row, and certifying evidence item rather than relying on closure prose.
...
- **0052 evidence/status:** `archive/tickets/0052FOUCONFOU-001.md` through `archive/tickets/0052FOUCONFOU-011.md` enumerate the current correction family across opaque session construction, closed command dispatch, replay authority reconstruction, TUI de-authority, declared-process effects, actor disposition census, sealed embodied/debug products, production-boundary negative fixtures, standing mutation disposition, and required CI checks. The 0052 rows should be cited for current defect-family closure; the 0048 acceptance, 0050 rows, and 0051 rows remain historical/prior-pass context.
- **0053 mitigation status:** The 0053 acceptance manifest requires each existing F5 finding label to carry an explicit status and evidence row. A residual can remain only as bounded `routed-forward` with owner, reason, next execution move, deadline, and failing check; otherwise it is non-pass. This mitigates incomplete correction closure without retiring the general risk.
