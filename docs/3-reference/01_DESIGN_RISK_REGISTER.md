...
- **Mitigation / guardrails:** Route every world-affecting ordinary-play change through command or action proposal, validation report, scheduling or resolution, event commit, projection rebuild, replay, and debug explanation. Keep debug/test metadata explicitly marked and non-diegetic. Human time controls must consume the same world-step pipeline and never become TUI-owned simulation.
- **Evidence to watch:** Direct component writes outside event application, state changes without validation reports, authored timed outcome chains, boundary code owning domain rules, tests that inspect current state without event causality, and human wait/continuation paths that bypass the shared world step.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md`, `crates/tracewake-core/tests/world_step_coordinator.rs`, and `crates/tracewake-tui/tests/playable_capability_parity.rs` record the shared core world-step transaction and measured TUI path for human time controls.
...
- **Mitigation / guardrails:** Require sealed holder-known contexts on planner, institution, language, lead, LOD, and view-model queries. Use actor-known projections for embodied UI. Test multi-possession scenarios, false beliefs, stale records, contradiction, rumor, wrong suspicion, adversarial hidden truth, and interval summaries built only from positive actor-known evidence.
- **Evidence to watch:** View models built from raw state then manually filtered, tests checking truth instead of holder/source/confidence, hidden event references in prompt packets, actor notebooks sourced from debug projections, and missed-event summaries that look like redacted omniscience.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md`, `crates/tracewake-core/tests/holder_known_interval_projection.rs`, `crates/tracewake-core/tests/salient_stop_actor_known.rs`, and `crates/tracewake-tui/tests/playable_capability_parity.rs` record positive actor-known interval deltas and measured holder-known output rather than raw-log redaction.
...
- **Mitigation / guardrails:** Use structured speech acts, deterministic templates, deterministic mocks, actor-filtered prompt packets, output validation, unsupported-claim rejection, and LLM-disabled operation as the baseline. Presentation may render or parse proposals; it may not author authority. Interval prose must render typed actor-known summary evidence rather than create elapsed-time facts.
- **Evidence to watch:** Test flakiness tied to live generation, generated records without speech/report events, prompt packets containing private truth, acceptance tests golden-locking LLM wording, UI text consumed as rules, and time-skip summaries whose prose is the only authority.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md` and `crates/tracewake-tui/tests/playable_capability_parity.rs` require measured typed interval/frontier evidence before rendered time-control or interval-summary text counts.
...
- **Mitigation / guardrails:** Keep debug view models visibly non-diegetic. Separate human/debug notes from actor notebooks. Route debug injections through explicit debug/test events. Test that embodied views, agents, language packets, institutions, and fixtures cannot consume debug-only projections. Human time controls must not reuse debug no-human-day reports as gameplay summaries.
- **Evidence to watch:** Shared projection types without mode tags, debug fields in embodied prompt packets, notebooks sourced from causal graph truth, tests requiring debug commands to complete normal flows, and embodied interval output sourced from debug step reports.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md` records the separation between duration/accounting debug evidence, loaded-world actor/process differential evidence, temporal replay evidence, and holder-known interval evidence.
...
- **Mitigation / guardrails:** Commit causally meaningful events with typed causes, content versions, random draw labels, and projection hints where useful. Use snapshots, summary events, projection rebuilds, event versioning, and upcasters without losing active ancestry. Otherwise-empty human world steps need replay-visible tick ancestry and deterministic interval projection rebuilds.
- **Evidence to watch:** Events per simulated day, replay time, projection rebuild time, old-fixture replay failures, missing causal references after compaction, active records or beliefs pointing to summarized-away detail, and empty time-control steps that advance current state without replay ancestry.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md`, `crates/tracewake-core/tests/replay_temporal_frontier.rs`, and `crates/tracewake-core/tests/world_step_coordinator.rs` record replay-visible `TimeAdvanced` ancestry and temporal frontier reconstruction for otherwise-empty and loaded-world steps.
...
- **Mitigation / guardrails:** Require semantic TUI action selection, actor-filtered view models, actor notebook, why-not explanations, transcript replay, debug/embodied separation, declared surface disposition, real-pipeline actor-filtered witnesses, Hop-1 goldens over view-model/projection coverage, and Hop-2 compile/source guards over renderer coverage. Time controls and interval summaries need the same two-hop parity treatment as other playable capabilities.
- **Evidence to watch:** Headless features marked accepted, no stable semantic action IDs, UI code enforcing world rules, acceptance demos that require debug mode, user-visible summaries sourced from truth, view-model capability entries without rendered witnesses, rendered controls without typed actor-known witnesses, missing Hop-1 golden coverage, weakened Hop-2 renderer guards, or time-control affordances proven only in headless/debug paths.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md` and `crates/tracewake-tui/tests/playable_capability_parity.rs` record measured core-to-view-to-render parity for time-control and interval-summary outputs, including adversarial measurement probes.
...
- **Mitigation / guardrails:** Implement needs, routines, interruptions, search, storage, work, sleep, eating, social contact, household expectations, and local reports as ordinary systems. Run no-human day fixtures from early phases, and compare human-driven ticks against no-human progression through the same loaded-world step.
- **Evidence to watch:** No-human event distribution, routine interruption traces, ordinary action failures, discovered absence without human intervention, records or reports created by unpossessed actors, and human wait/sleep/work paths that diverge from no-human duration completion or single-charge accounting.
- **0048 evidence:** `reports/0048_foundational_conformance_hardening_acceptance.md`, `crates/tracewake-core/tests/world_step_coordinator.rs`, and `crates/tracewake-core/tests/generative_lock.rs` record no-human/loaded-world actor and process work through the same scheduler transaction used by human time controls.
...
- **0053 evidence/status:** The current loaded-world/time-control reachability proof is the 0053 sealed-boundary replacement, not the historical 0048 harness-injected population, the narrower 0051 runtime row, or the 0052 public raw-bootstrap row. `LoadedWorldRuntime::from_bootstrap` is the production constructor from `ValidatedLoadedWorldBootstrap`; public `TuiApp::submit_semantic_action`, `TuiApp::advance_until`, and command-loop commands cross the TUI/core boundary through closed `RuntimeCommand` dispatch; debug/operator commands require `DebugSessionAuthority`; observed effects are committed actor/process events, restored eligibility, actor census changes, replay verdicts, sealed receipts, rendered output, and sealed interval products. `archive/tickets/0053FOUCONFIF-004.md`, `archive/tickets/0053FOUCONFIF-005.md`, `archive/tickets/0053FOUCONFIF-006.md`, and `archive/tickets/0053FOUCONFIF-007.md` record the sealed bootstrap, sealed interval product, token-gated debug/no-human command authority, and closed `food_source_fact_supersedes` mutation family. `archive/tickets/0053FOUCONFIF-003.md` records that branch-protection/ruleset governance must be API-proven, pending, or unverified; workflow topology alone is not pass evidence.
- **0053 mitigation status:** The 0053 manifest and wording-guard mechanism makes reachability and closure claims fail closed when `open`, `pending-governance`, unbounded `routed-forward`, `historical-only`, sampled, or observer-only evidence is counted as pass. Future 0053 acceptance rows must name the emitting path, status-manifest row, certifying evidence item, and any governance API transcript rather than relying on closure prose.
- **0054 status/evidence:** The sixth-pass F6-04/F6-06 machinery tightens the
  same risk by treating acceptance artifacts as computed read models over
  current exact-commit evidence. A closure row now needs actual artifact
  ingestion, independent-acceptance posture, and current mutation evidence with
...
- **0053 evidence/status:** `archive/tickets/0053FOUCONFIF-001.md` through `archive/tickets/0053FOUCONFIF-007.md` enumerate the current correction family across fail-closed acceptance status, below-foundation doctrine, governance API audit, sealed loaded bootstrap, sealed embodied interval/receipt products, token-gated debug/no-human authority, and the closed `food_source_fact_supersedes` mutation family. The 0053 rows supersede the 0052 routed-forward food-source status for this defect family; the 0048, 0050, 0051, and 0052 rows remain historical/prior-pass context where not explicitly superseded.
- **0053 mitigation status:** The 0053 acceptance manifest requires each existing F5 finding label to carry an explicit status and evidence row. A residual can remain only as bounded `routed-forward` with owner, reason, next execution move, deadline, and failing check; otherwise it is non-pass. The food-source family is now closed by public projection behavior tests plus a targeted mutation run, mitigating incomplete correction closure without retiring the general risk.
- **0054 status/evidence:** The sixth-pass acceptance machinery carries the
  current expected-finding manifest forward to F6 labels and requires each
  protected shortcut class to be represented by a current row. A `closed` row
...
- **0052 evidence/status:** The current non-vacuity posture for this seam is behavior-first: `generative_lock.rs` and `world_step_coordinator.rs` use the production bootstrap path, `command_loop_session.rs` and `embodied_flow.rs` cross public TUI commands into sealed receipts/rendering, `parity_adversarial.rs` fails when actor invocation, process invocation, temporal markers, or holder-known evidence are removed, and `negative_fixture_runner.rs` keeps due-work, actor-step outcome, TUI perception, embodied/debug product construction, and debug-to-embodied authority boundaries compile-fail under default and all-feature combinations. `archive/tickets/0052FOUCONFOU-010.md` records mutation sensitivity for the projection/interval/scheduler/runtime/TUI evidence; `archive/tickets/0052FOUCONFOU-011.md` records the required public-boundary lane and full-surface trigger. Source/topology scans remain secondary alarms and are not counted as behavior proof.
- **0053 mitigation status:** The 0053 governance audit job queries branch-protection and active branch-ruleset APIs and reports `pending/unverified` rather than pass when merge enforcement cannot be proven. The 0053 wording guard rejects branch-protection claims without an API or ruleset transcript, and `ci_workflow_guards.rs` remains a topology alarm only. The same non-vacuity rule now applies to sealed-boundary rows: `negative_fixture_runner.rs` must prove external clients cannot construct raw bootstraps, mutate/construct embodied interval products, or submit debug/operator commands without `DebugSessionAuthority`.
- **0054 status/evidence:** The sixth-pass PR-blocking mutation rule classifies
  `full-surface mutation trigger (lock layer)` as an alarm, not mutation proof.
  Guarded changes require actual in-diff mutation proof or a current
...
- **Private human clock relapse:** human wait, continuation, or advance-until controls advance only the possessed actor or a TUI-local timeline instead of the same authoritative world step used by no-human progression.
- **Duration/accounting split relapse:** human-started sleep/work closes through a pending queue, local client logic, or a second accounting path instead of the shared open-duration authority and one accounting classification per `(actor, need, tick)`.
- **Omniscient interval-summary relapse:** missed-event or interval output is generated from a raw world diff and redaction pass rather than positive actor-known evidence.
