...
Implemented the coordinator duration-lifecycle phase in `crates/tracewake-core/src/scheduler.rs`. `advance_world_one_tick` now accepts authoritative physical and agent state, preflights lifecycle work against scratch log/agent state before mutating the real log, appends the `TimeAdvanced` marker, then appends and applies duration lifecycle events generated by the existing sleep/work builders. Due sleep and work starts close through `build_sleep_completion_events` and `build_work_completion_events`; authorized sleep interruptions close through `build_sleep_interruption_events`. Work interruption remains out of scope because no work-interruption builder exists.

Added `AuthorizedSleepInterruption` as the typed authorization seam and lifecycle error variants that preserve fail-closed behavior for build/apply failures. The coordinator result now includes all appended event ids and the count of appended duration terminals.

Updated `crates/tracewake-core/tests/world_step_coordinator.rs` to cover due sleep completion with fatigue recovery, due work completion with output/need costs, authorized early sleep interruption with counted ticks, malformed lifecycle data fail-closed behavior, and duplicate terminal fail-closed behavior.

Verification:

1. `cargo fmt --all --check` — passed.
2. `cargo test -p tracewake-core --test world_step_coordinator` — passed.
3. `cargo test -p tracewake-core --test anti_regression_guards scheduler_apply_and_completion_paths_do_not_panic_on_log_derived_data` — passed after removing an `unwrap()` from the lifecycle ordering path.
