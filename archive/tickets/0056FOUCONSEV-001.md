...
2. Spec `archive/specs/0056_FOUNDATIONAL_CONFORMANCE_SEVENTH_HARDENING_*.md` §4.1 + §10.1 + §9 govern this ticket; the cross-crate authority topology is a spec-assigned implementer-recorded choice (see item 3). Driver: `reports/0047-foundational-hardening-research-report-seventh-pass.md` F7-01.
...
8. **Implementation-session correction (2026-06-28): topology (a) is mandatory for compile-time unforgeability.** A narrower token-parameter repair was rechecked against the live crate graph and is not viable: `tracewake-core` cannot name `tracewake-content::validate::FixtureValidationToken` without inverting the dependency, and Rust has no friend-crate visibility that would let a constructor be callable by `tracewake-content` but not by an outside crate. A public trait or feature-gated constructor would be forgeable by downstream crates (and cargo features are not security). Therefore options (b)/(c) are rejected for the current crate boundary unless they also move the validating authority into `tracewake-core`; this ticket's implementation must use topology (a), moving the validation/materialization authority that mints the runtime bootstrap into `tracewake-core` while preserving the workspace dependency direction (`content` may wrap/re-export core-owned content loading, but `core` must not depend on `content` or `tui`).
...
Use topology (a), per Assumption Reassessment item 8: move the validation/materialization authority that mints the runtime bootstrap into `tracewake-core`. `tracewake-content` may remain as a compatibility package for fixture modules, wrappers, or re-exports, but it must not own the final authority token or call a public raw-aggregate constructor. The rejected alternatives are (b)/(c) under the current crate boundary: a core token cannot be minted by `tracewake-content` without a public forgeable constructor, and a content token cannot be named by `tracewake-core` without violating the dependency direction.
...
- Move `LoadedFixture` / manifest / validation-token ownership for the runtime handoff into `tracewake-core`, or replace the content-owned `LoadedFixture` with a core-owned sealed proof object. If `tracewake-content::LoadedFixture` remains, it must be only a wrapper around the core-owned sealed proof and must not expose raw authoritative state.
- `ValidatedLoadedWorldBootstrap::from_validated_content(...)` is removed from the public API or changed to accept only the core-owned sealed product, not raw `PhysicalState` / `AgentState` / `EventLog` / `EpistemicProjection`.
