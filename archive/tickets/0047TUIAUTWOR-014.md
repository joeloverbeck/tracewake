...
Evidence:
- Added `human_sleep_completion_real_pipeline_witness` in `crates/tracewake-tui/tests/embodied_flow.rs`.
- The witness drives `do sleep.here`, rejected ordinary `wait`, `continue`, `bind-debug actor_tomas`, and `debug replay` through `run_command_loop`; it asserts reservation conflict, possessed duration terminal, actor-known interval summary, sleep need accounting, replay checksum parity, and absence of no-human-day witness text.
- Passed `cargo fmt --all --check`.
- Passed `cargo test -p tracewake-tui human_sleep_completion_real_pipeline_witness`.
- Passed `cargo test -p tracewake-tui sleep_completion`.
