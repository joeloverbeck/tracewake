...
Evidence-only ticket completed at implementation commit
...
- 20 named EPI suites across `tracewake-core`, `tracewake-content`, and `tracewake-tui`

Captured the effective configured census:

- `reports/0041_epi_cert_mutation_list_files.txt`: 54 files
- `reports/0041_epi_cert_mutation_list.txt`: 2774 mutants
- The final census includes the four historical survivor-bearing EPI files and the surrounding `epistemics/**` seam. The final census is 11 mutants larger than the historical 0040 census after 0041 code/test changes.

Ran the certifying standing mutation campaign without `--no-config`, filters,
`--in-diff`, `--iterate`, or exclusions:

```sh
cargo mutants --workspace --no-shuffle -o reports/0041_epi_cert_mutation_full.out
```

Final cargo-mutants summary:
...
- 0 missed
- 0 timeout
- start: `2026-06-19T21:14:12.33981041Z`
- end: `2026-06-19T23:39:18.066054579Z`

Produced `reports/0041_epi_cert_mutation_triage_register.md`. The register
reconciles all 30 historical identities: 29 still-generated identities were
caught in the full run, and the old `ContradictionKind::is_active` identity is
closed as source-changed/removed because the final code has no `is_active`
accessor and no final-run missed successor. There were no new final-run
survivors, no unresolved timeouts, no tool failures, no unsigned exceptions,
and no baseline-miss entries. `.cargo/mutants-baseline-misses.txt` remains
empty.
