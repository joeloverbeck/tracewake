...
- Added `crates/tracewake-core/src/need_accounting.rs` with a log-derived tick-regime classifier for awake/asleep/working ticks, consuming `is_duration_terminal`.
- Routed no-human passive need deltas through `classify_actor_tick_regimes`, so passive awake deltas use only awake-classified ticks.
- Routed sleep and work completion/interruption deltas through `classify_actor_tick_regimes_with_start`, with scheduler passing the live event log into both completion builders.
- Added `sleep_spanning_window_boundary_charges_each_tick_once_001` and registered it in the fixture roster/census.
- Added content ledger checks proving no duplicate awake/asleep/working need-regime charges for `no_human_day_001` and the new window-boundary fixture.
- Extended the anti-regression guard so scheduler passive deltas and duration completions must consume the classifier/log-aware paths.

Deviations from original plan:

- No checked-in golden fixture files required checksum repricing. The affected runs are generated by tests from fixture constructors, and the new ledger assertions are the durable review surface.
- `crates/tracewake-core/src/time.rs` did not need production changes; `passive_awake_need_deltas` remains the authored-rate arithmetic helper, while no-human scheduling now decides the classifier-derived elapsed tick count before calling it.
- The standalone `build_passive_need_delta_events` helper remains unchanged because it is not the no-human window path under audit and is only covered by its deterministic unit test.

Verification results:

- `cargo test -p tracewake-core need_accounting` — passed.
- `cargo test -p tracewake-core work_completion_emits_output_marker_and_need_costs` — passed.
- `cargo test -p tracewake-core sleep_completion_recovers_fatigue_by_elapsed_duration` — passed.
- `cargo test -p tracewake-content sleep_spanning_window_boundary_charges_each_tick_once` — passed.
- `cargo test -p tracewake-content no_human_need_ledger_has_no_duplicate_regime_charges` — passed.
- `cargo test -p tracewake-content golden` — passed.
- `cargo test -p tracewake-content --test fixtures_load` — passed after updating the explicit fixture census.
- `cargo test -p tracewake-core scheduler_never_direct_dispatches_primitive_action` — passed.
