...
2. Spec assumption: `archive/specs/0057_…_SPEC.md` §5 governs; it routes substance + home and explicitly does not pre-author wording or mint identifiers. Reassessment finding M5 confirmed these are the correct anchors (and that `docs/0-foundation/12:557` is the truth-firewall gate, not the behavioral-progress anchor — so this ticket touches the execution/architecture tiers only, not foundation). §5: "No constitutional-invariant amendment is required."
