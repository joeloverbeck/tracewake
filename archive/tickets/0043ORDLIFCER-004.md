...
Implemented the evidence and reconciliation portion of the ticket at frozen commit `d1cd9abda3e75f06570e4d60534c8db7c512720e`.

Preflight evidence was captured under `reports/0043_ord_life_cert_command_transcripts/004_preflight_*` and all preflight/named gate transcripts recorded `exit_status=0`, `supervisor_result=child_exit_0`. The final denominator artifacts are `reports/0043_ord_life_cert_mutation_list_files.txt` (60 lines; SHA-256 `510a1c626efda2b5118a6244184bf071c1a1fcda5237d9fa572861938046e50e`) and `reports/0043_ord_life_cert_mutation_list.txt` (2877 lines; SHA-256 `d9b430fbe111f5ea9a8c9db70bec032b3296444b38dac7a78aeecf672444c0a7`).

The certifying full campaign ran under `tools/supervise-command.sh` as `cargo mutants --workspace --no-shuffle -o reports/0043_ord_life_cert_mutation_full.out`. The supervisor transcript is `reports/0043_ord_life_cert_command_transcripts/004_full_campaign/` and recorded `exit_status=2`, `supervisor_result=child_nonzero_exit`; this was a cargo-mutants missed-mutant exit, not a launcher, baseline, timeout, or denominator-completion failure. The final outcome set was complete: `2877 mutants tested in 2h: 5 missed, 2257 caught, 615 unviable`, with zero timeouts. A sorted set comparison of `reports/0043_ord_life_cert_mutation_list.txt` against the union of final `caught.txt`, `missed.txt`, `timeout.txt`, and `unviable.txt` was empty.

The triage register is `reports/0043_ord_life_cert_mutation_triage_register.md`. The final missed manifest is `reports/0043_ord_life_cert_mutation_final_missed.txt` and is byte-identical to the campaign `missed.txt` (5 lines; SHA-256 `d2904d13cfc5404481434cb3d370a9e7b85a53c4f4e343a8b670eec86f1430bb`). The three historical `need_accounting.rs` survivor identities were caught in the completed full run at `caught.txt` lines 474, 478, and 481.

The completed run exposed five new behavior-relevant survivors: four in `crates/tracewake-core/src/actions/defs/movement.rs::build_move_event` and one in `crates/tracewake-core/src/actions/defs/wait.rs::is_autonomous_wait`. They were not laundered into `.cargo/mutants-baseline-misses.txt`; that file remains empty. Per this ticket's routing scope, the kill work was split into new follow-up tickets `tickets/0043ORDLIFCER-006.md` and `tickets/0043ORDLIFCER-007.md`, and `tickets/0043ORDLIFCER-005.md` now depends on those follow-ups before the replacement acceptance artifact can render a passing verdict.
